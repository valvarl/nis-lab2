...
void FEM<dim>::generate_mesh(std::vector<unsigned int> numberOfElements)
{
  ...
  double x_min = 0, x_max = 0.04, y_min = 0, y_max = 0.08, z_min = 0, z_max = 0.02;
...
void FEM<dim>::define_boundary_conds()
{
  ...
  auto process_nodes = [&](auto condition, auto action)
  {
    for (uint i = 0; i < totalNodes; ++i)
    {
      if (condition(nodeLocation[i]))
      {
        action(i, nodeLocation[i][0], nodeLocation[i][1]);
      }
    }
  };

  process_nodes([](const auto &node)
                { return std::abs(node[0]) < 1e-8; },
                [&](uint i, double x, double y)
                { boundary_values[i] = 300 * (1. + 1. / 3. * (y + nodeLocation[i][2])); });

  process_nodes([](const auto &node)
                { return std::abs(node[0] - 0.04) < 1e-8; },
                [&](uint i, double x, double y)
                { boundary_values[i] = 310 * (1. + 1. / 3. * (y + nodeLocation[i][2])); });
}
                  ...
                  for (unsigned int I = 0; I < dim; I++)
                  {
                    for (unsigned int J = 0; J < dim; J++)
                    {
                      ...
                      Klocal[A][B] += basis_gradient(A, quad_points[q1], quad_points[q2], quad_points[q3])[i] *
                                      ...
                                      invJacob[i][I] * kappa[I][J] * invJacob[j][J] * detJ *
                                      quad_points[q1], quad_points[q2], quad_points[q3];
                    }
                  }
                }
              }

...
void FEM<dim>::generate_mesh(std::vector<unsigned int> numberOfElements)
{
  ...
  double x_min = 0, x_max = 0.03, y_min = 0, y_max = 0.08;
...
void FEM<dim>::define_boundary_conds()
{
  ...
  // Define a lambda function to handle boundary condition assignments
  auto setBoundaryValue = [](double x, double y, double &boundary_value)
  {
    if (std::abs(y) < 1.e-8)
    {
      boundary_value = 300 * (1. + 1. / 3. * x);
    }
    else if (std::abs(y - 0.08) < 1.e-8)
    {
      boundary_value = 310 * (1. + 8. * x * x);
    }
  };
  ...
  {
    double x = nodeLocation[i][0];
    double y = nodeLocation[i][1];
    setBoundaryValue(x, y, boundary_values[i]);
  }
}
              ...
              for (unsigned int j = 0; j < dim; j++)
              {
                for (unsigned int I = 0; I < dim; I++)
                {
                  for (unsigned int J = 0; J < dim; J++)
                  {
                    ...
                    Klocal[A][B] += quad_weight[q1] * quad_weight[q2] * 
                    basis_gradient(A, quad_points[q1], quad_points[q2])[i] * 
                    basis_gradient(B, quad_points[q1], quad_points[q2])[j] * 
                    invJacob[i][I] * invJacob[j][J] * kappa[I][J] * detJ;
                  }
                }
              }
            }
